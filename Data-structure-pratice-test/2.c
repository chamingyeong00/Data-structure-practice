...
int isp[] = {0, 19, 12, 12, 13, 13, 13, 14, 15, 15, 0};
int icp[] = {20, 19, 12, 12, 13, 13, 13, 14, 15, 15, 0};
typedef enum{
    lparen, rparen, plus, minus, times, 
    divide, mod, power, no, e, operand
} precedence;
...
void postfix(char *new_expr){
    ...
    push(e);
    ft_print(stack, top);
    for (token = get_token(expr, &symbol, &n); token != e; token = get_token(expr, &symbol, &n))//주소 위치 보내기
    {
        if (token == operand)
        {
